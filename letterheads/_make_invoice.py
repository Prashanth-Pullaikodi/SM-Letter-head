"""Generate SandalMist Guest Invoice .docx templates (GST + non-GST) with placeholders."""
import os
...
GREEN = RGBColor(0x1F, 0x4D, 0x46); GOLD = RGBColor(0xC9, 0xA2, 0x4B)
CHARCOAL = RGBColor(0x33, 0x33, 0x33); GREY = RGBColor(0x70, 0x70, 0x70)
...
def build_invoice(include_gst, filename):
    doc = Document()
    ...
    run(title, "TAX INVOICE" if include_gst else "INVOICE", size=18, color=GREEN, bold=True, font="Georgia")
    ...
    gcell(g.rows[0].cells[0], "Guest Name", "GUEST_NAME"); gcell(g.rows[0].cells[1], "Meal Plan", "MEAL_PLAN")
    gcell(g.rows[1].cells[0], "Check-in", "CHECK_IN");     gcell(g.rows[1].cells[1], "Check-out", "CHECK_OUT")
    gcell(g.rows[2].cells[0], "Room", "ROOM_TYPE");        gcell(g.rows[2].cells[1], "No. of Guests", "NUM_GUESTS")
    ...
    rp = doc.add_paragraph(); rp.paragraph_format.space_before = Pt(4); run(rp, "{ROOM_DETAILS}", size=10)
    ...
    fp = doc.add_paragraph(); fp.paragraph_format.space_before = Pt(4); run(fp, "{FOOD_DETAILS}", size=10)
...
build_invoice(True, "05_Guest_Invoice_GST.docx")
build_invoice(False, "06_Guest_Invoice_NoGST.docx")
